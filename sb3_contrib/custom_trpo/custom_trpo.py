...
SelfCustomTRPO = TypeVar("SelfCustomTRPO", bound="CustomTRPO")
...
def total_kullback_leibler(p, q, c=2):
    # TODO: apply to deep learning
    kl = kullback_leibler(p, q)
    generator_grad_norm_squared = ((1 + p.logits) ** 2).sum(dim=-1)
    return 1 / th.sqrt(1 + c ** 2 * generator_grad_norm_squared) * kl


def itakura_saito(p, q):
    lp = p.logits - q.logits
    t = th.exp(lp) - lp - 1
    t[(q.probs == 0).expand_as(t)] = th.inf
    t[(p.probs == 0).expand_as(t)] = 0
    return t.sum(-1)
...
def total_itakura_saito(p, q, c=2):
    # TODO: apply to deep learning
    kl = kullback_leibler(p, q)
    generator_grad_norm_squared = ((-1/p.probs) ** 2).sum(dim=-1)
    return 1 / th.sqrt(1 + c ** 2 * generator_grad_norm_squared) * kl


def option_divergence(div: str, p, q):
    if div == "KL":
        return kullback_leibler(p, q)
    elif div == "tKL":
        return total_kullback_leibler(p, q)
    elif div == "IS":
        return itakura_saito(p, q)
    elif div == "tIS":
        return total_itakura_saito(p, q)
    else:
        return kullback_leibler(p, q)


def divergence(dist_true: Distribution, dist_pred: Distribution, div="KL") -> th.Tensor:
    """
    Wrapper for the PyTorch implementation of the full form KL Divergence

    :param dist_true: the p distribution
    :param dist_pred: the q distribution
    :param div: the divergence
    ...
    if isinstance(dist_pred, MultiCategoricalDistribution):
        ...
        return th.stack(
            [option_divergence(div, p, q) for p, q in zip(dist_true.distribution, dist_pred.distribution)],
            dim=1,
        ).sum(dim=1)
...
class CustomTRPO(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            # KL divergence
            kl_div = divergence(distribution, old_distribution, div=self.div).mean()
